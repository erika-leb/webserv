...
void Server::initFdListen(int fd, int port)
{
	...
	addr.sin_port = htons(port); //ICI VERIFIER QUE LE PORT EST OK

	if (bind(fd, (struct sockaddr*)&addr, sizeof(addr))) // utile car on est le serveur, inutile pour le client, equivaut a lister son numero dans l'annuaire
	{
		close(fd);
...
// 				{
// 					if (_events[i].data.fd == _fdListen[i]) // new incoming connection
// 					{
// 						cli_len = sizeof(cli);
// 						client_fd = accept(_fdListen[i], (struct sockaddr *)&cli, &cli_len);
// 						if (client_fd < 0)
// 						{
